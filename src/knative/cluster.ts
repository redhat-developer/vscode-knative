...
export class Cluster extends KnativeItem {
    ...
    static async logout(): Promise<string> {
        ...
        if (value === 'Logout') {
            return Cluster.kn.execute(KnAPI.odoLogout())
    ...
    static about(): void {
        Cluster.kn.executeInTerminal(KnAPI.printOdoVersion());
    }
    ...
    static async openshiftConsole(): Promise<void> {
        ...
        const versionInfo = await Cluster.kn.execute(KnAPI.getclusterVersion(), process.cwd(), false);
        if (versionInfo.error === null) {
            const routeObj = await Cluster.kn.execute(KnAPI.getOpenshiftClusterRoute());
            const spec = JSON.parse(routeObj.stdout).items[0].spec;
            consoleUrl = `${spec.port.targetPort}://${spec.host}`;
        } else {
            const serverUrl = await Cluster.kn.execute(KnAPI.showServerUrl());
    ...
    static async switchContext() {
        ...
        const choice = await window.showQuickPick(contextName, {placeHolder: "Select the new Knative context"});
        if (!choice) { return null; }
        return Promise.resolve()
            .then(() => Cluster.kn.execute(KnAPI.setOpenshiftContext(choice.label)))
    ...
    static async getUrl(): Promise<string | null> {
        ...
        return (choice.label === createUrl.label) ?
            await window.showInputBox({
                ...
                validateInput: (value: string) => Validation.validateUrl('Invalid URL provided', value)
            }) : choice.label;
    }
    ...
    private static async requestLoginConfirmation(skipConfirmation: boolean = false): Promise<string> {
        let response = 'Yes';
        if (!skipConfirmation && !await Cluster.kn.requireLogin()) {
    ...
    static async credentialsLogin(skipConfirmation: boolean = false): Promise<string> {
        ...
        const username =  (choice.label === addUser.label) ?
            await window.showInputBox({
                ...
                value: "",
                validateInput: (value: string) => Validation.emptyName('User name cannot be empty', value)
            }) : choice.label;
        ...
        return Progress.execFunctionWithProgress(`Login to the cluster: ${clusterURL}`,
            () => Cluster.kn.execute(KnAPI.odoLoginWithUsernamePassword(clusterURL, username, passwd))
    ...
    static async getUrlFromClipboard() {
        ...
        if (Validation.ocLoginCommandMatches(clipboard)) { return Validation.clusterURL(clipboard); }
        return null;
    }
    ...
    static async tokenLogin(skipConfirmation: boolean = false): Promise<string> {
        ...
        if (clusterUrlFromClipboard === clusterURL.trim()) {
            token = Validation.getToken(await Cluster.readFromClipboard());
        }
        ...
        return Progress.execFunctionWithProgress(`Login to the cluster: ${clusterURL}`,
            () => Cluster.kn.execute(KnAPI.odoLoginWithToken(clusterURL, ocToken))
            .then((result) => Cluster.loginMessage(clusterURL, result))
