...
export default class Service extends KnativeItem {
  ...
  static async create(context: KnativeTreeObject): Promise<string> {
    ...
    return Progress.execFunctionWithProgress(`Creating a new Service '${serviceName}'`, () =>
      Service.kn.createOSService(
        application,
        serviceTemplateName,
        serviceTemplatePlanName,
        serviceName.trim(),
      ),
    )
  ...
  static async del(treeItem: KnativeTreeObject): Promise<string> {
    ...
    if (!service) {
      const application: KnativeTreeObject = await Service.getKnativeCmdData(
        service,
        'From which Project you want to delete Service',
        'From which Application you want to delete Service',
      );
      if (application) {
        service = await window.showQuickPick(Service.getOsServiceNames(application), {
    ...
    if (service) {
      const answer = await window.showWarningMessage(
        `Do you want to delete Service '${service.getName()}'?`,
        'Yes',
        'Cancel',
      );
      if (answer === 'Yes') {
        return Progress.execFunctionWithProgress(
          `Deleting Service '${service.getName()}' from Application '${service
            .getParent()
            .getName()}'`,
          (progress) => Service.kn.deleteService(service),
        )
  ...
  static async describe(context: KnativeTreeObject): Promise<void> {
    ...
    if (!service) {
      ...
      if (application) {
        service = await window.showQuickPick(Service.getOsServiceNames(application), {
          placeHolder: 'Select Service you want to describe',
        });
      }
    }
    if (service) {
  ...
  static async getTemplate(service: KnativeTreeObject): Promise<string> {
    const result = await Service.kn.execute(
      KnAPI.getServiceTemplate(
        service
          .getParent()
          .getParent()
          .getName(),
