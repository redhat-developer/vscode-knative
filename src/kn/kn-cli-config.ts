...
export class KnCliConfig {
    ...
    static loadMetadata(requirements, platform): object {
    ...
    static async detectOrDownload(cmd: string): Promise<string> {
        ...
        if (toolLocation === undefined) {
            ...
            toolLocation = await KnCliConfig.selectTool(toolLocations, KnCliConfig.tools[cmd].versionRange);
            ...
            if (toolLocation === undefined) {
                ...
                const response = await vscode.window.showInformationMessage(
                    `Cannot find ${KnCliConfig.tools[cmd].description} ${KnCliConfig.tools[cmd].versionRangeLabel}.`, installRequest, 'Help', 'Cancel');
                ...
                if (response === installRequest) {
                    ...
                    do {
                        ...
                        if (sha256sum !== KnCliConfig.tools[cmd].sha256sum) {
                            ...
                            action = await vscode.window.showInformationMessage(`Checksum for downloaded ${KnCliConfig.tools[cmd].description} v${KnCliConfig.tools[cmd].version} is not correct.`, 'Download again', 'Cancel');
                        }
                    ...
                    if (action !== 'Cancel') {
                        if (toolDlLocation.endsWith('.zip') || toolDlLocation.endsWith('.tar.gz')) {
                            await Archive.unzip(toolDlLocation, path.resolve(Platform.getUserHomePath(), '.vs-kn'), KnCliConfig.tools[cmd].filePrefix);
                        } else if (toolDlLocation.endsWith('.gz')) {
                            await Archive.unzip(toolDlLocation, toolCacheLocation, KnCliConfig.tools[cmd].filePrefix);
                        }
                        fsExtra.removeSync(toolDlLocation);
                    ...
                    vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(`https://github.com/talamer/vscode-knative/blob/master/README.md#requirements`));
                }
            }
            if (toolLocation) {
    ...
    static async getVersion(location: string, cmd: string = path.parse(location).name): Promise<string> {
        let detectedVersion: string;
        if (fs.existsSync(location)) {
            // const version = new RegExp(`${cmd.toLocaleLowerCase()} v((([0-9]+)\\.([0-9]+)\\.([0-9]+)(?:-([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?)(?:\\+([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?).*`);
            const version = new RegExp(`Version:(\\s+)v((([0-9]+)\\.([0-9]+)\\.([0-9]+)(?:-([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?)(?:\\+([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?).*`);
