...
import { EventingTreeItem, compareNodes } from './eventingTreeItem';
...
export class EventingDataProvider implements TreeDataProvider<EventingTreeItem> {
  ...
  pollRefresh = (): void => {
    this.stopRefresh = setInterval(() => {
      ...
      // console.log(`EventingDataProvider.pollRefresh`);
      this.refresh();
  ...
  vfsListener = (event: FileChangeEvent[]): void => {
    // eslint-disable-next-line no-console
    console.log(`EventingDataProvider.vfsListener event ${event[0].type}, ${event[0].uri.path}`);
    this.refresh();
  };
  ...
  getChildren(element?: EventingTreeItem): ProviderResult<EventingTreeItem[]> {
    ...
    if (element && (element.contextValue === 'service' || element.contextValue === 'service_modified')) {
      ...
      children = this.getServices() as ProviderResult<EventingTreeItem[]>;
    }
    return children;
  }
   ...
   *
   * @param parentService: EventingTreeItem
   */
  private async getRevisionData(parentService: EventingTreeItem): Promise<CliExitData> {
   ...
   * @param parentService
   */
  public async getRevisions(parentService: EventingTreeItem): Promise<EventingTreeItem[]> {
    ...
    const revisionTreeObjects: EventingTreeItem[] = revisions.map<EventingTreeItem>((value) => {
   ...
   */
  public async getServices(): Promise<EventingTreeItem[]> {
    ...
    if (services.length === 0) {
      return [new EventingTreeItem(null, null, 'No Service Found', ContextType.NONE, TreeItemCollapsibleState.None, null, null)];
    }
  ...
  public async deleteFeature(node: EventingTreeItem): Promise<void> {
  ...
  async getUrl(): Promise<string | null> {
    ...
    //       validateInput: (value: string) => eventingDataProvider.validateUrl('Invalid URL provided', value),
    //     })
    //   : choice.label;
  ...
  public async addService(): Promise<EventingTreeItem[]> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(`eventingDataProvider.addService Error trying to read directory.\n ${err}`);
      // throw err;
      return null;
    }
    let filePath = '';
   ...
   * @param node Revision EventingTreeItem
   * @returns undefined if successful and null if failed
   */
  public async addTag(node: EventingTreeItem): Promise<EventingTreeItem[]> {
  ...
  public async getLocalYamlPathForNode(node: EventingTreeItem): Promise<string> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(`eventingDataProvider.getLocalYamlPathForNode Error trying to read directory.\n ${err}`);
      // throw err;
      return null;
    }
    let fileURI = '';
  ...
  public async isNodeModifiedLocally(serviceName: string): Promise<boolean> {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      // console.log(`eventingDataProvider.isNodeModifiedLocally Error trying to read directory.\n ${err}`);
      // throw err;
      return null;
    }
   ...
   * @param node Service EventingTreeItem
   */
  public async updateServiceFromYaml(node: EventingTreeItem): Promise<void> {
  ...
  public async deleteLocalYaml(node: EventingTreeItem): Promise<void> {
    const fileURI = await this.getLocalYamlPathForNode(node);
